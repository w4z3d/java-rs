...
pub fn execute(input: &str, output: String, no_remap_instructions: bool, no_correct_strings: bool) {
    let deobfuscation_options = Options {
        mode: Mode::Deobfuscate,
        correct_strings: !no_correct_strings,
        remap_instructions: !no_remap_instructions
    };

    if input.ends_with(".class") {
        process_class(&deobfuscation_options, JavaClass::read(&mut create_buffered_reader(input)).expect("cannot read class file")).expect("cannot deobfuscate class file").write(&mut create_buffered_writer(output)).expect("cannot write class file");
